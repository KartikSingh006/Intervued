...
export async function POST(request: Request) {
  try {
    ...
    // Dispatch transactional payload via Resend
    ...
    try {
      await resend.emails.send({
        ...
        subject: `Your TalentAI Interview Token: ${body.target_role}`,
        html: `
      ...
      console.log(`[RESEND SUCCESS] Sent secure token to ${body.candidate_email}`);
    } catch (emailError) {
      console.error("[RESEND ERROR] Failed to push email dispatch.", emailError);
    }
    
    return NextResponse.json({ 
      status: 'success', 
      token: token,
      message: 'Invitation generated, recorded, and dispatched via Resend.'
    });
  } catch (error: any) {
